...
Websites sometimes contain elements that take too much visual space or elements that are just too annoying!  
In most cases, these elements are created by JavaScript code.

Turning off JavaScript makes most websites unusable!  
Also, blocking scripts that render these annoying elements is a complex process and often results in a non-functional website.

There have been many tools that seem to delete elements — however, almost all of them simply **hide** the element you select.
...
It is a **mini-application**, an original work written from scratch, armed with intelligent state machines, contextual UI, dynamic behaviors, and performance-oriented architecture built for future expansion.

Unlike any other element-deletion (or blocking) tools, **Web Assassin completely removes** the element — including its code, data, and DOM footprint — as if it never existed on the website.
